...
high_priority_sb = Query("sb_r + abs(sb_r_err) - 0.6 * (r_mag - 14) > 17.8") | (~valid_sb)
...
relaxed_cut_sb = Query("sb_r + abs(sb_r_err) - 0.6 * (r_mag - 14) > 17.3") | (~valid_sb)
...
relaxed_cut_ug = Query("ug + abs(ug_err) > 0.4", "ug - abs(ug_err) < 2") | (~valid_u_mag) | (~valid_g_mag)
