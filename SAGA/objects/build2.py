...
def match_spectra_to_base_and_merge_duplicates(specs, base, debug=None):
    ...
    def get_tel_rank(tel, ranks=("MMT", "AAT", "PAL", "NSA", "_OTHERS", "SDSS", "ALFALF", "WIYN")):
        try:
...
def remove_shreds_near_spec_obj(base, nsa=None):
    ...
    for idx in chain(has_nsa_indices, has_spec_z_indices):
        ...
        if nearby_obj_count > 25 and remove_flag == 29:
            logging.warning(
                'More than 25 photo obj within ~ {:.3f}" of {} spec obj {} ({}, {})'.format(
                    remove_radius,
                    obj_this["TELNAME"],
                    obj_this["OBJID"],
                    obj_this["RA"],
...
def add_surface_brightness(base):
    base["sb_r"] = base["r_mag"] + 2.5 * np.log10(
        np.maximum(2.0 * np.pi * base["radius"] ** 2.0, 1.0e-40)
    )
    return base
