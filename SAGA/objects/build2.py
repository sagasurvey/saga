...
_spec_search_dz = 100.0/astropy.constants.c.to('km/s').value # pylint: disable=E1101
...
def merge_catalogs(**catalog_dict):
    ...
    group_id[regroup_mask] = get_fof_group_id(stacked_catalog[regroup_mask], 1.0)
...
def merge_spectra(specs):
    ...
    edge_mask = (np.ediff1d(specs['group_id']) > 0) | (np.ediff1d(specs['SPEC_Z']) > _spec_search_dz)
    group_id_edges = np.flatnonzero(np.hstack(([True], edge_mask, [True])))
    regroup_mask = np.zeros(len(specs), np.bool)
    for i, j in zip(group_id_edges[:-1], group_id_edges[1:]):
        if j - i > 1 and np.mean(specs['SPEC_Z'][i:j]) > 0.2:
...
def add_spectra(base, specs):
    ...
    for q, match_sep in (
            (Query('REMOVE == 0'), 1.0),
            (Query('REMOVE == 0', 'is_galaxy'), 3.0),
            (Query('REMOVE == 0') & ~Query('is_galaxy'), 3.0),
            (Query('REMOVE == 0', 'is_galaxy', 'r_mag < 17.0'), 20.0),
            (Query('REMOVE > 0'), 3.0),
    ):
        base_idx = np.flatnonzero(q.mask(base))
        not_matched_idx = np.flatnonzero(matched_idx == -1)
        idx, sep, _ = specs['coord'][not_matched_idx].match_to_catalog_sky(base['coord'][base_idx])
        matched_mask = (sep.arcsec < match_sep)
        matched_idx[not_matched_idx[matched_mask]] = base_idx[idx[matched_mask]]

    specs['matched_idx'] = matched_idx
    del matched_idx, base_idx, idx, sep, matched_mask, specs['coord']
